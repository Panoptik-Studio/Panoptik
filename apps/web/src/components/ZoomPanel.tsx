  ...
  if (!project) {
    return (
      <div className="border-b bg-white p-4" style={{ borderColor: "#ebebeb" }}>
        <h3 className="mb-1 text-[13px] font-semibold" style={{ color: "#171717", letterSpacing: "-0.02em" }}>Zoom</h3>
        <p className="font-mono text-[11px]" style={{ color: "#888" }}>Load a clip to add zooms. Click the preview while paused.</p>
      </div>
    );
  }
  ...
  return (
    <div className="border-b bg-white p-4" style={{ borderColor: "#ebebeb" }}>
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-[13px] font-semibold" style={{ color: "#171717", letterSpacing: "-0.02em" }}>Zoom</h3>
        <span className="rounded-full bg-[#fafafa] px-2 py-0.5 font-mono text-[10px]" style={{ color: "#888", border: "1px solid #ebebeb" }}>{allZooms.length} points</span>
      </div>

      <button
        onClick={() => addZoomPoint({ t: currentTime, to: { scale: 2.2, x: 0.5, y: 0.5 }, dur: 0.7, ease: "easeInOutCubic" })}
        className="flex w-full items-center justify-center gap-1.5 rounded-full px-4 py-2 text-xs font-medium text-white transition-colors"
      ...
      <p className="mt-1.5 text-center font-mono text-[10px]" style={{ color: "#888" }}>or click the preview while paused · drag focal to reposition</p>
